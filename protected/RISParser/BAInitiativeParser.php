...
class BAInitiativeParser extends RISParser
{
    ...
    public function parse(int $id): ?Antrag
    {
        ...
        if ($changed) {
            ...
            echo "BA-Initiative $id: Verändert: " . $aenderungen . "\n";
    ...
    public function parseUpdate(): void
    {
        ...
        $crit->condition = "typ='" . addslashes(Antrag::TYP_BA_INITIATIVE) . "' AND status != 'erledigt' AND gestellt_am > NOW() - INTERVAL 2 YEAR AND ((TO_DAYS(bearbeitungsfrist)-TO_DAYS(CURRENT_DATE()) < 14 AND TO_DAYS(bearbeitungsfrist)-TO_DAYS(CURRENT_DATE()) > -14) OR ((TO_DAYS(CURRENT_DATE()) - TO_DAYS(gestellt_am)) % 3) = 0)";
    ...
    public function parseMonth(int $year, int $month): array
    {
        ...
        $html = $this->browserBasedDowloader->downloadDocumentTypeListForPeriod(BrowserBasedDowloader::DOCUMENT_BA_INITIATIVE, $from, $to);

        $parsedObjects = StadtratsantragListEntry::parseHtmlList($html);


        echo count($parsedObjects) . " BA-Initiativen gefunden\n";
