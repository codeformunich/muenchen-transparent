...
$("li.strIn").each(function(){
    var data = [];
    for (var i = 0; i < referate.length; i++) {
        data.push({
            value: 0,
            color: css_basic_colors[i],
            highlight: css_basic_colors[i],
            label: referate[i]
        });
    }
    ...
    function add_value(index) {
        if (-1 < index && index < data.length) {
            data[index].value += 1;
        } else {
            console.log("Warnung bei Diagramzeichnen:");
            console.log(index);
            console.log($(this));
        }
    }

    for (current of $(this).find(".antraege-data").children().toArray()) {
        var text = $(current).find(".antrag-referat").text();
        
        add_value(referate.indexOf(text));
    };

    var chart = $(this).find("canvas").get(0).getContext("2d");
    new Chart(chart).Pie(data, {responsive: false, animation: false});
    //new Chart(chart).Bar({"labels": [], "datasets": data}, {responsive: true, animation: false});
});
});
</script>
        ...
        <div class="col-sm-9">
            ...
            <ul class="strIn_liste">
                ...
                foreach ($personen as $strIn) {
                    ...
                    <canvas class="personen-antraege-diagram"></canvas>
                    
                    <ul class="hidden antraege-data">
                        <?
                        $antraege = $strIn->antraege(array('limit' => 200));
                        foreach ($antraege as $antrag) {
                            echo "<li>";
                            echo "<span class='antrag-name'   >" . CHtml::encode($antrag->getLink())           . "</span>";
                            echo "<span class='antrag-datum'  >" . RISTools::datumstring($antrag->gestellt_am) . "</span>";
                            echo "<span class='antrag-referat'>" . CHtml::encode($antrag->referat)             . "</span>";
                            echo "</li>\n";
                        } ?>
                    </ul>
                    <?
