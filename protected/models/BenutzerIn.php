...
class BenutzerIn extends CActiveRecord
{
    ...
    {
        return self::model()->findAllByAttributes(["email_bestaetigt" => "1"], ["order" => "email"]);
    }
    ...
    public static function createBenutzerIn($email, $password = "")
    {
        ...
        $benutzerIn->pwd_enc                       = ($password != "" ? self::create_hash($password) : "");
    ...
    public function emailBestaetigen($code)
    {
        ...
        if ($this->pwd_enc == "") $this->pwd_enc = self::create_hash($code);
    ...
    public function resetPasswordDo($code, $new_pw)
    {
        ...
        $this->pwd_enc         = self::create_hash($new_pw);
    ...
    public static function heuteZuBenachrichtigendeBenutzerInnen()
    {
        ...
        $benutzerInnen = self::model()->findAllByAttributes(["email_bestaetigt" => 1]);
        $todo          = [];
    ...
    public static function getByFeedCode($code)
    {
        ...
        $benutzerIn = self::model()->findByPk($x[0]);
        if (!$benutzerIn) return;
