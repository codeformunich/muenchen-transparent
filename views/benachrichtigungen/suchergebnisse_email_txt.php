...
if (count($data["antraege"]) > 0) {
	...
	foreach ($data["antraege"] as $dat) {
		...
		echo "- \"" . $name . "\n";
		echo "  " . trim(Url::to("antraege/anzeigen", array("id" => $antrag->id)), ".") . "\n";
		echo implode("\n", $dokumente_strs);
		if (count($queries) == 1) {
...
foreach ($data["termine"] as $dat) {
	...
	if (strlen($name) > 80) $name = mb_substr($name, 0, 78) . "...";
	echo "- \"" . $name . "\n";
	echo "  " . trim(Url::to("termine/anzeigen", array("id" => $termin->id)), ".") . "\n";
	echo implode("\n", $dokumente_strs);
	if (count($queries) == 1) {
		echo "\n    Gefunden über: \"" . $queries[0] . "\"\n";
	} else {
...
?>
Falls du diese Benachrichtigung nicht mehr erhalten willst, kannst du sie unter <?php echo trim(Url::to("benachrichtigungen/index", ["code" => $benutzerIn->getBenachrichtigungAbmeldenCode()]), "."); ?> abbestellen.
