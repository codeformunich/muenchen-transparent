...
<section class="well suchergebnisse">
    ...
    foreach ($facet as $value => $count) if ($count > 0) {
        $str = "<li><a href='" . RISTools::bracketEscape(Html::encode($krits->cloneKrits()->addAntragTypKrit($value)->getUrl())) . "'>";
    ...
    foreach ($facet as $value => $count) if ($count > 0) {
        ...
        $str = "<li><a href='" . RISTools::bracketEscape(Html::encode($krits->cloneKrits()->addWahlperiodeKrit($value)->getUrl())) . "'>";
        $str .= $value . ' (' . $count . ')';
        $str .= "</a></li>";
        $wahlperiode[] = $str;
    }
